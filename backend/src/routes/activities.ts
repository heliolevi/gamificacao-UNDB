...
activitiesRouter.post("/", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
...
activitiesRouter.get("/", requireAuth, asyncHandler(async (_req, res) => {
  const activities = await prisma.activity.findMany();
  res.json(activities.map((a) => ({ ...a, qrPayload: `ACT:${a.id}` })));
}));

activitiesRouter.get("/:id", requireAuth, asyncHandler(async (req, res) => {
